...
/* ── Carte navigation projet avec thumbnail ─────────────────── */
...
function ProjectNavCard({ project: p, direction }) {
  ...
  return (
    ...
    >
      {/* Thumbnail gauche pour "précédent" */}
      {isPrev && (
      ...
      <div className={`flex-1 flex flex-col justify-center gap-1 p-4 min-w-0 ${!isPrev ? 'items-end text-right' : ''}`}>
        ...
        <p className="text-sm font-bold text-slate-200 group-hover:text-violet-300 transition-colors leading-snug truncate">
          {p.title}
        </p>
        {p.type && (
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-8 sm:py-12 space-y-10 sm:space-y-14">
      ...
      <Reveal as="header" className="relative">
        ...
        {/* Description */}
        <p className="relative text-base sm:text-lg text-slate-400 leading-relaxed max-w-2xl mb-6">
          {project.description}
        </p>
              ...
              {project.gallery.map((img, i) => (
                <button
                  ...
                  className={`relative shrink-0 w-20 h-14 sm:w-24 sm:h-16 rounded-lg overflow-hidden transition-all duration-200 focus:outline-none ring-offset-slate-950 ${
                    activeImage === i ? 'ring-2 ring-violet-500 ring-offset-2' : 'opacity-50 hover:opacity-75'
                  }`}
                >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* ── Fonctionnalités & apprentissages (col 1-2) ── */}
        <Reveal className="lg:col-span-2">
          <div className="h-full bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
            {/* Header de la card */}
            <div className="flex items-center gap-2.5 px-5 py-4 border-b border-slate-800">
              ...
              <div>
                <h2 className="text-sm font-semibold text-slate-100">Fonctionnalités & apprentissages</h2>
      ...
      <Reveal>
        <div className="space-y-5 pt-10 border-t border-slate-800/80">
          ...
          {/* Cartes prev / next avec thumbnails */}
